  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-30 mix-blend-overlay"
        style={{ backgroundImage: "radial-gradient(circle at 20% 20%, white 0%, transparent 40%), radial-gradient(circle at 80% 70%, white 0%, transparent 50%)" }}
      />
      ...
      <div className="max-w-7xl mx-auto container-px">
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <SectionHeader
            ...
            description="Production-grade tutorials and architectures from real enterprise projects."
            align="left"
          />
        ...
        <div className="mt-12 grid lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="p-6 lg:p-8">
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground inline-flex items-center gap-1"><Clock className="h-3 w-3" />{featured.readingTime}</span>
              </div>
              ...
              <p className="mt-4 text-xs text-muted-foreground">{featured.date} · {featured.author}</p>
            </div>
          </Link>
          ...
          <div className="space-y-4">
            <div className="p-5 rounded-2xl border border-border/70 bg-card">
              ...
              <div className="mt-4 space-y-4">
                {rest.slice(0, 3).map((a, i) => (
                  <Link key={a.slug} to="/blog/$slug" params={{ slug: a.slug }} className="group flex gap-3">
                    <span className="text-2xl font-semibold text-muted-foreground/40 w-6">{i + 1}</span>
                    <div className="min-w-0">
                      <p className="text-sm font-medium group-hover:text-primary transition-colors line-clamp-2">{a.title}</p>
                      <p className="mt-1 text-xs text-muted-foreground">{a.category} · {a.readingTime}</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div className="p-5 rounded-2xl border border-border/70 bg-card">
              <p className="text-xs font-medium uppercase tracking-[0.16em] text-primary">Latest tutorial</p>
              <Link to="/blog/$slug" params={{ slug: rest[3]?.slug ?? rest[0].slug }} className="group block mt-3">
                <Thumb seed={3} className="h-28 rounded-lg" />
                <p className="mt-3 text-sm font-medium group-hover:text-primary transition-colors">{rest[3]?.title}</p>
