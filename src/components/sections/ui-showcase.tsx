          ...
          <div className="lg:col-span-8 rounded-2xl glass-card overflow-hidden p-6 hover:shadow-[var(--shadow-elevated)] transition-shadow">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-1.5">
                {["7d", "30d", "90d", "12m"].map((t, i) => (
                  <span
                    key={t}
                    className={`text-[11px] px-2 py-1 rounded-md ${i === 1 ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="mt-4 rounded-lg bg-background/60 border border-border/60 p-4">
              <svg viewBox="0 0 400 120" className="w-full h-32">
                ...
                <path
                  d="M0,90 C40,80 70,60 110,65 C160,72 190,30 240,38 C290,46 320,20 400,15 L400,120 L0,120 Z"
                  fill="url(#g2)"
                />
                <path
                  d="M0,90 C40,80 70,60 110,65 C160,72 190,30 240,38 C290,46 320,20 400,15"
                  fill="none"
                  stroke="oklch(0.7 0.16 235)"
                  strokeWidth="1.5"
                />
              </svg>
            </div>
          ...
          <div className="lg:col-span-7 rounded-2xl glass-card p-6">
            ...
            <div className="mt-4 rounded-lg border border-border/60 overflow-hidden">
              ...
              {[
                ...
                <div
                  key={row.r}
                  className="grid grid-cols-4 px-4 py-2.5 border-t border-border/60 text-sm items-center"
                >
                  <span className="font-mono text-xs">{row.r}</span>
                  <span className="text-muted-foreground">{row.o}</span>
                  <span>
                    <span
                      className={`text-[10.5px] px-2 py-0.5 rounded-full ${row.c === "primary" ? "bg-primary/15 text-primary" : row.c === "success" ? "bg-emerald-500/15 text-emerald-500" : "bg-secondary text-muted-foreground"}`}
                    >
                      {row.s}
                    </span>
                  </span>
          ...
          <div className="lg:col-span-5 rounded-2xl glass-card p-6">
            ...
            <div className="mt-4 grid grid-cols-[180px_1fr] gap-3 h-56">
              <div className="rounded-lg bg-background/60 border border-border/60 p-2 space-y-1">
                {["Overview", "Apps", "Flows", "Analytics", "Settings", "Members", "Billing"].map(
                  (it, i) => (
                    <div
                      key={it}
                      className={`text-xs px-2.5 py-1.5 rounded-md ${i === 1 ? "bg-primary/15 text-primary" : "text-muted-foreground"}`}
                    >
                      {it}
                    </div>
                  ),
              ...
              <div className="rounded-lg bg-background/60 border border-border/60 p-3 grid grid-cols-2 gap-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-md bg-gradient-to-br from-primary/15 to-primary-glow/10 border border-border/40"
                  />
                ))}
              </div>
            </div>
